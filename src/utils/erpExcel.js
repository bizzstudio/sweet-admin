...
// ── הסקת טיפוסים ב-CSV, ולמה יש דרך לכבות אותה ──
//
// ‏xlsx מנסה לזהות תאריכים בתאי CSV ומפרש אותם **בסדר האמריקאי**. נמדד על
// קובץ "היסטוריה ללקוח" מההנהח"ש:
//
//     "30/06/2026 10:41"  ->  נשאר מחרוזת   (30 אינו חודש אפשרי)
//     "08/06/2026 11:32"  ->  46240.48      (6 באוגוסט, במקום 8 ביוני)
//     "04/12/2025 11:01"  ->  45759.46      (12 באפריל, במקום 4 בדצמבר)
//
// כלומר באותה עמודה מעורבבים מחרוזות ומספרים, וכל תאריך שהיום בו קטן מ-13
// מתהפך — בשקט, בלי שורה פסולה ובלי אזהרה. קובץ ההנהח"ש כותב יום/חודש/שנה.
//
// ‏rawCells מכבה את ההסקה ומחזיר את התאים כפי שנכתבו בקובץ, כדי שהמפענח
// שיודע מה הפורמט יקרא אותם בעצמו. הדגל הוא **opt-in ולא ברירת מחדל**: יבוא
// המוצרים והלקוחות מסתמך על ההמרה הקיימת, ושינוי גורף כאן היה משנה בשקט את
// אופן הקריאה של כל אחד מהם.
//
// הדגל חל על CSV בלבד. בקובץ xlsx התאריך שמור כתא תאריך אמיתי ולא כטקסט,
// ולכן אין שם ניחוש שצריך לכבות.
 ...
 * @param {boolean} [options.rawCells=false] - ב-CSV: להחזיר תאים בלי הסקת
 *        טיפוסים (ראה ההסבר למעלה). המספרים חוזרים אז כמחרוזות, ו-toNumber
 *        ממיר אותם כרגיל.
 */
export const readSheetGrid = (file, { rawCells = false } = {}) =>
